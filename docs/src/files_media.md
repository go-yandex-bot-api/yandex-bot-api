...
3. **Корректные MIME-типы (Content-Type):**
   API Яндекс Мессенджера строго проверяет заголовок `Content-Type` для отправляемых файлов. Для методов `SendImage` и `SendGallery` сервер требует валидный тип `image/*` (например, `image/png`, `image/jpeg`). При отправке `application/octet-stream` API возвращает ошибку `415 Unsupported Media Type`. Библиотека автоматически определяет MIME-тип по расширению файла, устанавливает корректные заголовки для multipart-частей, а также позволяет при необходимости передать явный `ContentType` через структуру `types.RequestFile`.
